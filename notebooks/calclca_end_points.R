...
dir_plot_out       <- paste0(getwd(),  '/plots/end_points/')                              # output


#scenarios
scenario_files <- list.files(path = dir_outtab, pattern = 'results_dalys_SSP2')
lca_files      <- list.files(path = dir_lca, pattern = 'lca_REMIND_monetization_coal_exit')
...
lca_remind <- na.omit(lca_remind) %>% 
  group_by(scenario,category, end_category, period, tech) %>% 
